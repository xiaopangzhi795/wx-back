...
public class HttpClientUtil {
    ...
    public static Map doPost(String url, Map<String, String> header, JSONObject param, int connectionTimeOut, int readTimeOut, String charSet,boolean isHeader) throws IOException {
        return doPost(url, header, param.toJSONString(), connectionTimeOut, readTimeOut, charSet, isHeader, null, null, null, null, null);
    }

    public static Map doPostString(String url, String param, int connTimeOut, int readTimeOut, String charSet, boolean isHeader) throws IOException {
        Map<String, String> header = getDefaultHeader();
        return doPost(url, header, param, connTimeOut, readTimeOut, charSet, isHeader, null, null, null, null, null);
    }
     ...
     * @throws IOException
     */
    public static Map doPost(String url, Map<String, String> header, String param, int connectionTimeOut, int readTimeOut, String charSet,boolean isHeader,String hostName, Integer port, String scheme, String username, String pass) throws IOException {
        log.info("get request url: {}, params: {}", url, JSON.toJSONString(param));
        CloseableHttpClient httpclient = null;
        CloseableHttpResponse response = null;
        CookieStore cookieStore = new BasicCookieStore();
        ...
        try {
            ...
            try {
                if (isHeader) {
                    data = JSONObject.parseObject(param);
                }
            } catch (Exception e) {
                data = null;
            }
            URI uri = getUri(isHeader ? data : null, url, charSet);
            // 创建http GET请求
            post = new HttpPost(uri);
            post = init(post, header);
            if (!isHeader && param != null && StringUtils.isNotBlank(param)) {
                ContentType contentType = getContentType(charSet, header.get("Content-Type"));
                post.setEntity(new StringEntity(param, contentType));
            }
